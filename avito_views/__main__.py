...
async def reset(настройки, доводы) -> None:
    # Ноль — законное значение: «вернуть всё, что сейчас в работе», поэтому сравниваем с
    # None, а не проверяем на правдивость.
    через = доводы.stale if доводы.stale is not None else настройки.зависла_через
    очередь = Очередь(настройки.очередь_dsn)
    try:
        итог = await очередь.сбросить_зависшие(через)
    finally:
        await очередь.закрыть()
    лог.info("возвращено в очередь (взятых больше %.0f c назад): %s", через, итог)
